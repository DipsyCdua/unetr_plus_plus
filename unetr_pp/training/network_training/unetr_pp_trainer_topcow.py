...
from unetr_pp.network_architecture.topcow.unetr_pp_topcow import UNETR_PP
...
class unetr_pp_trainer_topcow(Trainer_synapse):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        current_stage = self.stage if self.stage is not None else (1 if len(self.plans['plans_per_stage']) == 2 else 0)
        self.crop_size = np.array(self.plans['plans_per_stage'][current_stage]['patch_size']).astype(int)
    ...
    def initialize(self, training=True, force_load_plans=False):
        if not self.was_initialized:
            ...
            current_stage = self.stage if self.stage is not None else (
                1 if len(self.plans['plans_per_stage']) == 2 else 0
            )
            # Match the TopCoW UNETR++ decoder outputs: 1/1, 1/2x1/4x1/4, 1/4x1/8x1/8.
            self.plans['plans_per_stage'][current_stage]['pool_op_kernel_sizes'] = [
                [2, 4, 4],
                [2, 2, 2],
                [2, 2, 2],
            ]
            ...
            self.folder_with_preprocessed_data = join(
                ...
                self.plans['data_identifier'] + "_stage%d" % current_stage,
            )
    ...
    def initialize_network(self):
        ...
        input_res = (1, *[int(i) for i in self.crop_size])
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.fp16:
            with autocast():
                ...
                del data
                if self.deep_supervision and isinstance(output, (tuple, list)):
                    target = self._ensure_deep_supervision_target(target, output)
    ...
    def _ensure_deep_supervision_target(self, target, output):
        if isinstance(target, (tuple, list)):
            return target

        targets = [target]
        for out in output[1:]:
            if list(out.shape[2:]) == list(target.shape[2:]):
                targets.append(target)
                continue

            resized = F.interpolate(target.float(), size=out.shape[2:], mode="nearest")
    ...
    def do_split(self):
        if self.fold == "all":
            ...
            if self.fold < len(splits):
                ...
                self.print_to_log_file("INFO: You requested fold %d for training but splits contain only %d folds. "
                                       "I am now creating a random (but seeded) 80:20 split!" % (self.fold, len(splits)))
